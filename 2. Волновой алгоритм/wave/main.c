...
int main() {
	...
	int n = 0, res = 0; // -1 - не дошли, 1 - дошли
	...
	while (!res) {
		...
		for (int i = 0; i < N; i++) {
			for (int j = 0; j < N; j++) {
				int what = (n == 0 ? S : n); // Если n равен 0, то S, иначе n
				if (a[i][j] != what) continue;

				if (j > 0 && a[i][j - 1] == F) { res = 1; break; }
				if (i < N - 1 && a[i + 1][j] == F) { res = 1; break; }
				if (j < N - 1 && a[i][j + 1] == F) { res = 1; break; }
				if (i > 0 && a[i - 1][j] == F) { res = 1; break; }

				if (j > 0 && a[i][j - 1] == 0) { a[i][j - 1] = n + 1; k++; }
				if (i < N - 1 && a[i + 1][j] == 0) { a[i + 1][j] = n + 1; k++; }
				if (j < N - 1 && a[i][j + 1] == 0) { a[i][j + 1] = n + 1; k++; }
				if (i > 0 && a[i - 1][j] == 0) { a[i - 1][j] = n + 1; k++; }
			}
			if (res == 1) break;
		}
		if (res == 1) break;
